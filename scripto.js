document.addEventListener('DOMContentLoaded', () => { 
    ...
    let isScrolling = false; // Flag to prevent scroll interruption

    // --- Glitch Fix: Backdrop-Filter Redraw Helper ---
    // Forces a browser re-render to fix backdrop-filter not appearing on scroll.
    ...
    const handleScrollEvents = () => { 
        ...
        if (window.scrollY > 50) { 
            // Only add class and redraw if not already present (optimization)
            if (navbar && !navbar.classList.contains('scrolled')) {
                navbar.classList.add('scrolled'); 
                forceRedraw(navbar); // Force redraw for blur fix
            }
            if (navMenu && !navMenu.classList.contains('scrolled')) {
                navMenu.classList.add('scrolled'); 
                forceRedraw(navMenu); // Force redraw for blur fix
            }
        } else {
